<template>
    <div style="background-color: #fff">
        ...
        <div class="standard__wrap">
            <div v-for="item in joinStore.module3" class="container">
                <h3>{{ item.title }}</h3>
...
const getBackgroundPicture = async () => {
    try {
        const result = await getMobileNav({ decorateSn: "investmentCriteria" });
        Object.assign(joinStore.module3, result?.data);
    } catch (error) {
        console.log(error);
    } finally {
        joinStore.module3Loaded = true;
    }
};
if (joinStore.module3Loaded === false) {
