// **---------------------------------------------------------------------+
// ** 文件 --
// **---------------------------------------------------------------------+
// ** 版权所有：江西佰商科技有限公司. 官网：https://www.tigshop.com
// **---------------------------------------------------------------------+
// ** 作者：Tigshop团队，yq@tigshop.com
// **---------------------------------------------------------------------+
// ** 提示：Tigshop商城系统为非免费商用系统，未经授权，严禁使用、修改、发布
// **---------------------------------------------------------------------+

package com.tigshop.bean.vo.salesman;

import com.tigshop.bean.model.salesman.Salesman;
import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;

import java.util.List;

/**
 * 分销商品VO
 *
 * @author Tigshop团队
 * @create 2024年12月03日 16:56
 */
@Data
@Schema(description = "分销商品参数")
public class SalesmanProductVO {

    @Schema(description = "分销商品ID")
    private Integer salesmanProductId;

    @Schema(description = "商品ID")
    private Integer productId;

    @Schema(description = "是否参与分销，0为不参与，1为参与")
    private Integer isJoin;

    @Schema(description = "佣金计算方式，1为默认，2为自定义比例，3为自定义金额")
    private Integer commissionType;

    @Schema(description = "佣金数据")
    private List<CommissionData> commissionData;

    @Schema(description = "添加时间")
    private String addTime;

    @Schema(description = "更新时间")
    private String updateTime;

    @Schema(description = "店铺ID")
    private Integer shopId;

    @Schema(description = "产品佣金信息")
    private ProductCommission productCommission;

    @Schema(description = "下单时分销员对象")
    private Salesman salesman;

    @Data
    @Schema(description = "产品佣金信息内部类")
    public static class ProductCommission {
        @Schema(description = "一级分销员佣金")
        private String productCommission;

        @Schema(description = "子佣金")
        private String subCommission;
    }

    @Data
    public static class CommissionData {
        @Schema(description = "佣金类型")
        private List<LevelData> levelArr;

        @Data
        public static class LevelData {
            @Schema(description = "等级")
            private Integer level;

            @Schema(description = "rate")
            private String rate;

            @Schema(description = "下级rate")
            private String downSalesmanRate;
        }
    }
}