...
import { WithdrawFormState } from "@/types/vendor/capitalfund/withdraw.d";
import { auditWithdraw, getWithdrawDetail, uploadPayVoucher } from "@/api/vendor/capitalfund/withdraw";
...
const onSubmit = async () => {
    ...
    try {
        ...
        if (info.value.status == 4) {
            let {paymentVoucher} = formState.value
            const voucherResult = await uploadPayVoucher({ id: id.value, paymentVoucher });
