                            ...
                            <el-table :data="filterState" :loading="loading" :total="total" row-key="shopId" @sort-change="onSortChange">
                                ...
                                <el-table-column label="变动资金账户">
                                    <template #default="{ row }">
                                        ...
                                        <!-- <span v-if="(row.type == 1 || row.type == 4) && row.vendorMoney > 0" style="color: #0000ff">+{{ row.vendorMoney }}</span>
...
import { getAdminVendorLogList } from "@/api/vendor/capitalfund/dashboard";
...
const props = defineProps({ vendorId: { type: Number, default: 0 } });
...
const {
    ...
    apiFunction: getAdminVendorLogList,
    idKey: "vendorId",
    defaultParams: {
        page: 1,
