                    ...
                    <a-spin :spinning="loading">
                        <div style="padding-right: 15px; box-sizing: border-box">
                            ...
                            >
                                ...
                                <el-table-column label="装修页面名称" prop="decorateTitle"></el-table-column>
                                <template #empty>
...
const selectRow = (selection: FilterState[], val: FilterState) => {
    if (!props.isMultiple) {
        ...
        selectedIds.value.push(val.decorateId);
        emit("okType", selectedIds.value.length > 0);
    }
    linkSelectData.value = {
        path: "decorate",
        label: val.decorateTitle,
        name: val.decorateTitle,
        id: val.decorateId,
        data: {
            name: val.decorateTitle,
            path: "decorate",
...
const isSelectable = (row: FilterState) => {
    // 排除重复项
    return !props.selectedIds.includes(row.decorateId); // Column configuration not to be checked
};
// 弹窗回调
const onFormSubmit = () => {
    // 弹窗确认按钮提交
