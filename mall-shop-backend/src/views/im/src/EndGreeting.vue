<template>
    <div class="index-m_preferenceSwitch_1TP3q">
        <div class="index-m_header_2mdaa"><span class="index-m_title_180SF">客服离线提醒语</span>
            <el-switch
                class="switch-pl"
                v-model="formState.activate"
            />
        </div>
        <div class="index-m_desc_Erwpt">所有客服均不在线时，会发送这个回复</div>
    </div>
...
const formState: any = ref({
    activate: false,
    sendText: false,
    replyContent: "我们现在都不在呢，等明天早上看到消息会第一时间回复的"
});
...
const onSubmit = async () => {
    try {
        const result = await saveImConfig({ code: "off_work", data: formState.value });
    } catch (error: any) {
...
const fetchData = async () => {
    try {
        const result = await getImConfig({ code: "off_work" });
        if(result){
            Object.assign(
                formState.value,
                result
